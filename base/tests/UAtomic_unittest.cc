...
void *setFuc_0x55_atomicInt_getAndSet(void *arg)
{
    ...
    switch(*c)
    {
        case 1:
            ...
            {
                AtomicUInt8 _ui8 = atomicInt_getAndSet(ui8, 0x55);
                if(_ui8 != 0x55)
                {
                    EXPECT_EQ(0xaa, _ui8);
                    //printf("--- [%s]:i:%x, _ui8:%x\n",__func__,i,_ui8);
        ...
        case 2:
            for(i = 0; i < TestTimes; i++)
            {
                AtomicUInt16 _ui16 = atomicInt_getAndSet(ui16, 0x5555);
                if(_ui16 != 0x5555)
                    EXPECT_EQ(0xaaaa, _ui16);
                else
                    EXPECT_EQ(0x5555, _ui16);
            }
            break;
        case 3:
            for(i = 0; i < TestTimes; i++)
            {
                AtomicUInt32 _ui32 = atomicInt_getAndSet(ui32, 0x55555555);
                if(_ui32 != 0x55555555)
                    EXPECT_EQ(0xaaaaaaaa, _ui32);
                else
        ...
        case 4:
            for(i = 0; i < TestTimes; i++)
            {
                AtomicUInt64 _ui64 = atomicInt_getAndSet(ui64, 0x5555555555555555);
                if(_ui64 != 0x5555555555555555)
                    EXPECT_EQ(0xaaaaaaaaaaaaaaaa, _ui64);
                else
...
void *setFuc_0xaa_atomicInt_getAndSet(void *arg)
{
    ...
    switch(*c)
    {
        case 1:
            ...
            {
                AtomicUInt8 _ui8 = atomicInt_getAndSet(ui8, 0xaa);
                if(_ui8 != 0xaa)
                {
                    EXPECT_EQ(0x55, _ui8);
                    //printf("--- [%s]:i:%x, _ui8:%x\n",__func__,i,_ui8);
        ...
        case 2:
            for(int i = 0; i < TestTimes; i++)
            {
                AtomicUInt16 _ui16 = atomicInt_getAndSet(ui16, 0xaaaa);
                if(_ui16 != 0xaaaa)
                    EXPECT_EQ(0x5555, _ui16);
                else
                    EXPECT_EQ(0xaaaa, _ui16);
        ...
        case 3:
            for(int i = 0; i < TestTimes; i++)
            {
                AtomicUInt32 _ui32 = atomicInt_getAndSet(ui32, 0xaaaaaaaa);
                if(_ui32 != 0xaaaaaaaa)
                    EXPECT_EQ(static_cast<AtomicUInt32>(0x55555555), _ui32);
                else
                    EXPECT_EQ(0xaaaaaaaa, _ui32);
        ...
        case 4:
            for(int i = 0; i < TestTimes; i++)
            {
                AtomicUInt64 _ui64 = atomicInt_getAndSet(ui64, 0xaaaaaaaaaaaaaaaa);
                if(_ui64 != 0xaaaaaaaaaaaaaaaa)
                    EXPECT_EQ(static_cast<AtomicUInt64>(0x5555555555555555), _ui64);
                else
                    EXPECT_EQ(0xaaaaaaaaaaaaaaaa, _ui64);
            }
